...
const FlightSearch = () => {
  ...
  const handleCityInput = (input, setter, fieldType) => {
    setter(input);
  ...
  // Function to get random flights for the selected date
  const getRandomFlightForDate = (userSelectedDate) => {
    const filtered = flightsData.filter(
      (flight) => flight.departureDate === userSelectedDate
    );
    const randomFlight = filtered[Math.floor(Math.random() * filtered.length)];
    return randomFlight;
  };
    ...
    <main className="min-h-full w-screen flex-col z-30 flex items-left justify-center">
      <div className="lg:w-[80vw] w-full flex flex-col items-left justify-center gap-8">
        ...
        <div className="w-full flex flex-col gap-4 ">
          ...
          <ScrollArea className="max-h-[25vh] lg:max-h-[35vh] 2xl:max-h-full overflow-y-auto z-10  no-scrollbar">
